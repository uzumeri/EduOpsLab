...
- **Management System Metastandards** ([year]) — A personal whitepaper analyzing the structural architecture of ISO-family management system standards (quality, environmental, safety) and their common structural elements. The special-process analogy in LP-3 — that some educational outcomes are not detectable at the point of production — draws directly on the logic of ISO 9001 process-validation requirements. Not peer-reviewed. Archived at `reference/ManagementSysgtemMetastandards.pdf` (filename typo preserved as-is). → LP-3
